...
export class PersonaService {
  ...
  getPersonaFindId(idPersona: string): Observable<persona> {
    this.perosonaDoc = this.getCollectionPersona().doc(idPersona)

    return this.perosonaDoc.snapshotChanges().pipe(map(documentoAction => {
      const personaData = (documentoAction.payload.data() as persona)
