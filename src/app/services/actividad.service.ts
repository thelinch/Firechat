...
export class ActividadService {
  ...
  getAllIncidenciaFindIdActividad(
    idAvctividad: string,
    idIndice: string
  ): Observable<incidencias[]> {
    return this.getCollectionActividades(idIndice)
      .doc(idAvctividad)
      .collection<incidencias>("incidencias")
      .snapshotChanges()
      .pipe(
        map(actions =>
          actions.map(documentoListaIncidencia => {
  ...
  getCollectionActividades(
    idIndice: string
  ): AngularFirestoreCollection<actividades> {
    return this.afs.collection<actividades>(Colecciones.actividades, ref =>
      ref.where("idIndice", "==", idIndice)
    );
  }
  ...
  getUltimoEstadoIdActividad(idActividad: string, idIndice: string) {
    this.getCollectionActividades(idIndice)
      .doc(idActividad)
      .collection("estado");
  }
  getResultadoICA(
  ...
  getAllRespuestaFindIdActividad(
    idActividad: string,
    fecha_fin: Date,
    fecha_inicio: Date
    ...
    return this.afs
      ...
      .pipe(
        map(actions =>
          actions.map(documentoResultados => {
  ...
  getAllActividadFindIdIndice(
    idIndice: string,
    fecha_inicio
  ): Observable<actividades[]> {
    console.log(fecha_inicio);
    return this.afs
      .collection<actividades>(Colecciones.actividades, ref =>
        ref
          .orderBy("fecha_inicio")
          .where("fecha_inicio", ">=", new Date(fecha_inicio))
          .where("estado", "==", true)
