...
export class ResultadoService {
  ...
  constructor(
    ...
    private sendMessaging: SendMessageService
  ) {}
  getAllResultadoFindIdActividad(
    idActividad: string
  ): Observable<DocumentData[]> {
    return this.afs
      .collection<resultado>(Colecciones.actividades)
      .doc(idActividad)
  ...
  getAllParametroResultadoIncumplido(): Observable<resultado[]> {
    return this.afs
      .collection("resultadosIncumplidos")
      .snapshotChanges()
      .pipe(
        map(listResult =>
  ...
  guardarResultado(
    ...
    return Observable.create(observer => {
      listaParametro.forEach(parametro => {
        ...
        this.getCumplimientoOIncumplimiento(
          parametro.resultado,
          parametro,
          resultado
        );
        ...
        if (!resultado.cumplio || resultado.riesgo == "Muy Alto") {
          ...
          const titleNotification = "ICA ";
          const bodyNotification = `Parametro Incumplido ${resultado.parametro.nombre} Resultado ${resultado.resultado}`;
          const data = {
            lt: resultado.lat,
            ln: resultado.lng,
            urlImage: "No hay"
          };
          this.sendMessaging.sendMessagingDevices(
            bodyNotification,
            titleNotification,
            data,
            resultado.persona
  ...
  saveResultado(resultado: resultado, idActividad: string) {
    this.afs
      .collection(Colecciones.actividades)
      .doc(idActividad)
      .collection("resultado")
      .add(resultado);
  }
  getEstadisticaParametroFinIdActividad(
    idActividad: string
  ): Observable<any[]> {
    return Observable.create(observer => {
      ...
      this.afs
        ...
        .pipe(
          map(actions =>
            actions.map(documentoResultado => {
        ...
        .subscribe(listaResultado => {
          ...
          listaResultado.forEach(resultado => {
            listaResultado
              .filter(
                resultadoNew =>
                  resultadoNew.parametro.nombre == resultado.parametro.nombre
              )
              .forEach(r => {
                dataResultado.push(parseFloat(r.resultado.toString()));
              });
            let indice = data.findIndex(
              re => re.label == resultado.parametro.nombre
            );
            if (indice != -1) {
              data[indice].data = dataResultado;
            } else {
              data.push({
                label: resultado.parametro.nombre,
                data: dataResultado,
  ...
  getCumplimientoOIncumplimiento(
    resultado: number,
    parametro: parametro,
    resultadoObjeto: resultado
  ) {
  ...
  getRiesgo(resultado: number, parametro: parametro): any {
    ...
    let elemento = this.arrayRiesgo.find(
      elemento => elemento.parametro == parametro.nombreCorto
    );
    if (elemento) {
      console.log(elemento);
      riesgo = elemento.listaRiesgo.find(
        riesgo =>
          resultado < riesgo.rango[0]["inicio"] &&
          resultado >= riesgo.rango[0]["fin"]
      );
    }
    return riesgo;
  }
