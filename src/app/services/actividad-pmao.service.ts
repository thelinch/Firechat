...
export class ActividadPmaoService {
  ...
  constructor(
    ...
    private sendMessagingService: SendMessageService
  ) {}

  saveActividadPMAO(
    idIndice: string,
    actividad: actividadPMAO
  ): Observable<boolean> {
    return Observable.create(obsever => {
      actividad.personaRegistro = JSON.parse(
      ...
      actividad.subActividades.forEach(subActividad => {
        ...
        const titleNotification = "PMAO ";
        const bodyNotification =
          "Nueva Propuesta del contratista" + actividad.personaRegistro.nombre;
        const data = {
          lt: 0,
          ln: 0,
          urlImage: "No hay"
        };
        this.sendMessagingService.sendMessagingDevices(
          bodyNotification,
          titleNotification,
          data,
          actividad.personaRegistro
        );
      });
      this.afs
        .collection("indice")
        .doc(idIndice)
        .collection("actividadPMAO")
        .add(actividad);
      obsever.next(true);
    });
  }
  saveActividadEjecucionPMAOFindIdActividad(
    actividad: actividadPMAO,
    idIndice: string,
    execution: executionActivityPMAO
  ): Observable<boolean> {
  ...
  updateSubActividad(
    ...
    return Observable.create(observer => {
      this.afs
        ...
        .doc(idIndice)
        .collection("actividadPMAO")
        .doc(actividadPmao.id)
        .update({ subActividades: actividadPmao.subActividades });
      observer.next(true);
    });
  }
  getAllActividadPMAO(idIndice: string): Observable<actividadPMAO[]> {
    return this.afs
      ...
      .pipe(
        map(actions =>
          actions.map(documentoActividad => {
  ...
  getAllActividadFromName(
    idIndice: string,
    nombre: string
  ): Observable<actividadPMAO[]> {
    return this.afs
      .collection("indice")
      .doc(idIndice)
      .collection("actividadPMAO", f => f.where("nombre", "==", nombre))
      .snapshotChanges()
      .pipe(
        map(actions =>
          actions.map(documentoActividad => {
  ...
  updateEstadoActividadFindIdIndice(
    actividad: actividadPMAO,
    idIndice: string
  ) {
    ...
    this.afs
      ...
      .collection("actividadPMAO")
      .doc(actividad.id)
      .update({ estadoActividad: actividad.estadoActividad });
  }
  updateObservacionesFindActividadAndId(
    actividad: actividadPMAO,
    idIndice: string
  ) {
  ...
  setValoracionFindIdActividad(
    ...
    return Observable.create(observer => {
      this.afs
        ...
        .collection("actividadPMAO")
        .doc(actividad.id)
        .update({ valoracion: actividad.valoracion });
      observer.next(true);
    });
  }
}
